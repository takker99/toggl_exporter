...
function getLastModifyDatetime() {
    ...
    file = file.next();
  let data = JSON.parse(file.getAs("application/octet-stream").getDataAsString());
...
function getTimeEntries(unix_timestamp) {
  let uri = 'https://www.toggl.com/api/v8/time_entries' + '?' + 'start_date=' + encodeURIComponent(Moment.moment(unix_timestamp, 'X').format());
...
function recordActivityLog(description, started_at, ended_at) {
    var calendar = CalendarApp.getCalendarById(getTargetCalendar());
    ...
    try {
        ...
        if (time_entries) {
            ...
            for (var i = 0; i < time_entries.length; i++) {
                ...
                let project_data = getProjectData(record.pid);
                var project_name = project_data.name || '';
        let activity_log = [(record.description || '名称なし'), project_name].filter(function(e){return e}).join(" : ");
            ...
            if (last_stop_datetime) {
                putLastModifyDatetime(
                    (
                        parseInt(
                            Moment.moment(last_stop_datetime).format('X'),
                            10
                        ) + 1
                    ).toFixed()
                );
